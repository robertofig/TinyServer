...
ParseHttpHeader(string InBuffer, ts_request* Request, ts_request_body* Body)
{
    ...
    if (!Request->FirstHeaderOffset)
    {
        ...
        if (EqualStrings(Verb, StrLit("GET"))) Request->Verb = HttpVerb_Get;
        else if (EqualStrings(Verb, StrLit("HEAD"))) Request->Verb = HttpVerb_Head;
        else if (EqualStrings(Verb, StrLit("POST"))) Request->Verb = HttpVerb_Post;
        else if (EqualStrings(Verb, StrLit("PUT"))) Request->Verb = HttpVerb_Put;
        else if (EqualStrings(Verb, StrLit("DELETE"))) Request->Verb = HttpVerb_Delete;
        else if (EqualStrings(Verb, StrLit("CONNECT"))) Request->Verb = HttpVerb_Connect;
        else if (EqualStrings(Verb, StrLit("OPTIONS"))) Request->Verb = HttpVerb_Options;
        else if (EqualStrings(Verb, StrLit("TRACE"))) Request->Verb = HttpVerb_Trace;
        else if (EqualStrings(Verb, StrLit("PATCH"))) Request->Verb = HttpVerb_Patch;
        else
        {
            return HttpParse_HeaderInvalid;
        }
        ...
        // Parse Version
        string Version = String(Line.Base + LineReadCur, Line.WriteCur - LineReadCur, 0, EC_ASCII);
        StripCLRF(Version);
        
        if (Version.WriteCur == 0) Request->Version = HttpVersion_09;
        else if (EqualStrings(Version, StrLit("HTTP/1.0"))) Request->Version = HttpVersion_10;
        else if (EqualStrings(Version, StrLit("HTTP/1.1"))) Request->Version = HttpVersion_11;
        else if (EqualStrings(Version, StrLit("HTTP/2.0"))) Request->Version = HttpVersion_20;
        else
        {
            return HttpParse_HeaderInvalid;
        }
    ...
    while (ReadCur != INVALID_IDX)
    {
        ...
        string Value = String(Line.Base + LineReadCur, Line.WriteCur - LineReadCur, 0, EC_ASCII);
...
ParseFormData(ts_request_body RequestBody)
{
    ...
    // Field boundary is evaluated with a -- before it, so we include it before the string in the header to facilitate search.
    
    string EntityType = String(RequestBody.ContentType, RequestBody.ContentTypeSize, 0, EC_ASCII);
    char* BoundaryPtr = (char*)CharInString('=', EntityType, RETURN_PTR_FIND);
    *BoundaryPtr-- = '-';
    *BoundaryPtr   = '-';
    string Boundary = String(BoundaryPtr, (EntityType.Base+EntityType.WriteCur) - BoundaryPtr, 0, EC_ASCII);
    ...
    while (FormIsValid && ReadCur < Body.WriteCur)
    {
        if (ParseStage == FormParse_FirstLine)
        {
            ...
            if (Line.Base)
            {
                string Content = StrLit("Content-Disposition: form-data");
                if (CompareStrings(Line, Content, Content.WriteCur, RETURN_BOOL))
                {
                        ...
                        && FieldName.WriteCur <= U16_MAX)
                    {
                        ...
                        // If field is file, format is [name="FieldName";filename="File.ext"].
        ...
        else if (ParseStage == FormParse_SecondLine)
        {
            ...
            if (Line.Base)
            {
                ...
                string Content = StrLit("Content-Type");
                if (CompareStrings(Line, Content, Content.WriteCur, RETURN_BOOL))
                {
                    usz LineCur = Content.WriteCur;
                    
                    // Ignores Content-Type (it can later be retrieved from file ext).
                    // Checks to see if file has encoding information, format is [charset=Encoding].
        ...
        else if (ParseStage == FormParse_Data)
        {
            ...
            if ((DataEnd = BufferInBuffer(Boundary.Buffer, RawData, RETURN_IDX_FIND)) != INVALID_IDX)
            {
                         ...
                         || Body.Base[ReadCur] == '\n')
                {
                    ...
                    Parser.NextFieldOffset = (u16)((Body.Base + ReadCur) - ((char*)RawData.Base + DataEnd));
...
GetFormFieldByName(ts_multiform Form, char* TargetName)
{
    ...
    for (usz Count = 0; Count < Form.FieldCount; Count++)
    {
        ...
        string FieldName = String(Ptr + TargetField->FieldNameOffset, TargetField->FieldNameLen, 0, EC_ASCII);
...
FormatTimeIMF(datetime TimeFormat, string* Dst)
{
    ...
    if (Dst->Size >= (Dst->WriteCur + IMF_DATE_LENGTH))
    {
        ...
        AppendStringToString(StrLit(", "), Dst);
        if (TimeFormat.Day < 10) AppendStringToString(StrLit("0"), Dst);
        ...
        AppendStringToString(StrLit(" "), Dst);
        if (TimeFormat.Hour < 10) AppendStringToString(StrLit("0"), Dst);
        AppendIntToString(TimeFormat.Hour, Dst);
        AppendStringToString(StrLit(" "), Dst);
        if (TimeFormat.Minute < 10) AppendStringToString(StrLit("0"), Dst);
        AppendIntToString(TimeFormat.Minute, Dst);
        AppendStringToString(StrLit(" "), Dst);
        if (TimeFormat.Second < 10) AppendStringToString(StrLit("0"), Dst);
        AppendIntToString(TimeFormat.Second, Dst);
        AppendStringToString(StrLit(" GMT"), Dst);
...
external void
CraftHttpResponseHeader(ts_response* Response, string* Header)
{
    ...
    {
        case HttpVersion_09: Version = StrLit("HTTP/0.9 "); break;
        case HttpVersion_10: Version = StrLit("HTTP/1.0 "); break;
        default:
        case HttpVersion_11: Version = StrLit("HTTP/1.1 "); break;
        case HttpVersion_20: Version = StrLit("HTTP/2.0 "); break;
    }
    ...
    {
        case 100: StatusCode = StrLit("100 Continue"); break;
        case 101: StatusCode = StrLit("101 Switching Protocol"); break;
        case 200: StatusCode = StrLit("200 OK"); break;
        case 201: StatusCode = StrLit("201 Created"); break;
        case 202: StatusCode = StrLit("202 Accepted"); break;
        case 203: StatusCode = StrLit("203 Non-Authoritative Information"); break;
        case 204: StatusCode = StrLit("204 No Content"); break;
        case 205: StatusCode = StrLit("205 Reset Content"); break;
        case 300: StatusCode = StrLit("300 Multiple Choices"); break;
        case 301: StatusCode = StrLit("301 Moved Permanently"); break;
        case 302: StatusCode = StrLit("302 Found"); break;
        case 303: StatusCode = StrLit("303 See Other"); break;
        case 304: StatusCode = StrLit("304 No Modified"); break;
        case 305: StatusCode = StrLit("305 Use Proxy"); break;
        case 307: StatusCode = StrLit("307 Temporary Redirect"); break;
        case 308: StatusCode = StrLit("308 Permanent Redirect"); break;
        case 400: StatusCode = StrLit("400 Bad Request"); break;
        case 401: StatusCode = StrLit("401 Unauthorized"); break;
        case 402: StatusCode = StrLit("402 Payment Required"); break;
        case 403: StatusCode = StrLit("403 Forbidden"); break;
        case 404: StatusCode = StrLit("404 Not Found"); break;
        case 405: StatusCode = StrLit("405 Method Not Allowed"); break;
        case 406: StatusCode = StrLit("406 Not Acceptable"); break;
        case 407: StatusCode = StrLit("407 Proxy Authorization Required"); break;
        case 408: StatusCode = StrLit("408 Request Timeout"); break;
        case 409: StatusCode = StrLit("409 Conflict"); break;
        case 410: StatusCode = StrLit("410 Gone"); break;
        case 411: StatusCode = StrLit("411 Length Required"); break;
        case 412: StatusCode = StrLit("412 Precondition Failed"); break;
        case 413: StatusCode = StrLit("413 Payload Too Large"); break;
        case 414: StatusCode = StrLit("414 URI Too Long"); break;
        case 415: StatusCode = StrLit("415 Unsupported Media Type"); break;
        case 416: StatusCode = StrLit("416 Range Not Satisfiable"); break;
        case 417: StatusCode = StrLit("417 Expectation Failed"); break;
        case 418: StatusCode = StrLit("418 I'm a Teapot"); break;
        case 421: StatusCode = StrLit("421 Misdirected Request"); break;
        case 425: StatusCode = StrLit("425 Too Early"); break;
        case 426: StatusCode = StrLit("426 Update Required"); break;
        case 428: StatusCode = StrLit("428 Precondition Required"); break;
        case 429: StatusCode = StrLit("429 Too Many Requests"); break;
        case 431: StatusCode = StrLit("431 Request Header Fields Too Large"); break;
        case 440: StatusCode = StrLit("440 Login Timeout"); break;
        case 451: StatusCode = StrLit("451 Unavailable for Legal Reasons"); break;
        case 500: StatusCode = StrLit("500 Internal Server Error"); break;
        case 501: StatusCode = StrLit("501 Not Implemented"); break;
        case 502: StatusCode = StrLit("502 Bad Gateway"); break;
        case 503: StatusCode = StrLit("503 Service Unavailable"); break;
        case 504: StatusCode = StrLit("504 Gateway Timeout"); break;
        case 505: StatusCode = StrLit("505 HTTP Version Not Supported"); break;
        case 507: StatusCode = StrLit("507 Insufficient Storage"); break;
        case 510: StatusCode = StrLit("510 Not Extended"); break;
        case 511: StatusCode = StrLit("511 Network Authentication Required"); break;
    ...
    AppendStringToString(StrLit("Date: "), Header);
    FormatTimeIMF(CurrentSystemTime(), Header);
    AppendStringToString(LineBreak, Header);
    
    AppendStringToString(StrLit("Server: "), Header);
    ...
    AppendStringToString(StrLit("Content-Length: "), Header);
    ...
    {
        AppendStringToString(StrLit("Content-Type: "), Header);
        AppendArrayToString(Response->PayloadType, Header);
        AppendStringToString(LineBreak, Header);
    }
    
    if (Response->CookiesSize == 0)
    {
        // OBS: If there are cookies to be sent, final newline goes in cookies buffer, else it's here.
        AppendStringToString(LineBreak, Header);
    }
